...
#define Yaw_sita_weight 0.5f 		//位置环权重
#define Yaw_minipc_sita_weight 0.003f	//位置环权重
...
void Yaw_task(void const *pvParameters)
{
  ...
  for(;;)
  {
		...
		//motor_info[5].set_voltage = pid_calc(&motor_pid[5], target_speed[5], motor_info[5].rotor_speed);//速度环转电流环
		motor_info[6].set_voltage = pid_calc(&motor_pid[6], target_speed[6], 20 * ins_data.gyro[2]);//用陀螺仪的角速度（rad/s -> r/min）
		Yaw_can_send();
    osDelay(1);
  }
