...
#include "Yaw_task.h"
//================================================PID结构体定义================================================//
pid_struct_t motor_pid[8];	
pid_struct_t motor_pid_sita[8];
pid_struct_t motor_pid_can_2[8];	
pid_struct_t motor_pid_sita_can_2[8];
...
float pid_calc_sita_span_left(pid_struct_t *pid,float target, float respond)//带死区处理
{
  pid->ref = target;
  pid->fdb = respond;
	pid->err[1] = pid->err[0];
...
float pid_calc_sita_span_right(pid_struct_t *pid,float target, float respond)//带死区处理
{
  pid->ref = target;
  pid->fdb = respond;
	pid->err[1] = pid->err[0];
