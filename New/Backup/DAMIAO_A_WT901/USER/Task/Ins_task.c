...
uint8_t Tx[8] = {0x50,0x03,0x00,0x37,0x00,0x09,0x39,0x83};
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	if(huart->Instance == USART2)
	{
		...
		{
			GX = (((short)Rx[3]<<8)|Rx[4])/32768.0*2000; //度/s
			GY = (((short)Rx[5]<<8)|Rx[6])/32768.0*2000;
			GZ = (((short)Rx[7]<<8)|Rx[8])/32768.0*2000;
			
			GX = GX*60/360.0; //Roll RPM
			GY = GY*60/360.0; //Pitch
			GZ = GZ*60/360.0; //Yaw
			
			Roll = (((short)Rx[15]<<8)|Rx[16])/32768.0*180; //角度制 0~360
			Pitch = (((short)Rx[17]<<8)|Rx[18])/32768.0*180;
			Yaw = (((short)Rx[19]<<8)|Rx[20])/32768.0*180;
